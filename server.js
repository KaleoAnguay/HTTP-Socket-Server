...
const server = net.createServer((request) => {
  ...
  request.on('data', (data) => {
    ...
    let headersAndStatus = `HTTP/1.1 200 OK
...
${indexHTML}`;
  ...
  if(path === '/helium.html') {
    headersAndStatus = `HTTP/1.1 200 OK
Content-Type: text/html
...
${css}`;
  ...
  if(path !== '/hydrogen.html' && path !== '/helium.html' && path !== '/' && path !== '/favicon.ico' && path !== '/css/styles.css') {
    headersAndStatus = `HTTP/1.1 200 OK
Content-Type: text/html

${error}`;
  }
    request.write(headersAndStatus);
    request.end();
...
const css = `@import url(http://fonts.googleapis.com/css?family=Open+Sans|Roboto+Slab);
